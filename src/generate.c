...
int
nmea_gen_GPGGA(char *buff, int buff_sz, nmeaGPGGA *pack)
{
	return nmea_printf(
	    buff,
	    buff_sz,
...
int
nmea_gen_GPGSA(char *buff, int buff_sz, nmeaGPGSA *pack)
{
	return nmea_printf(
	    buff,
	    buff_sz,
...
int
nmea_gen_GPGSV(char *buff, int buff_sz, nmeaGPGSV *pack)
{
	return nmea_printf(buff,
			   buff_sz,
			   "$GPGSV,%1d,%1d,%02d,"
			   "%02d,%02d,%03d,%02d,"
			   "%02d,%02d,%03d,%02d,"
			   ...
			   pack->sat_data[0].id,
			   pack->sat_data[0].elv,
			   pack->sat_data[0].azimuth,
			   pack->sat_data[0].sig,
			   pack->sat_data[1].id,
			   pack->sat_data[1].elv,
			   pack->sat_data[1].azimuth,
			   pack->sat_data[1].sig,
			   pack->sat_data[2].id,
			   pack->sat_data[2].elv,
			   pack->sat_data[2].azimuth,
			   pack->sat_data[2].sig,
			   pack->sat_data[3].id,
			   pack->sat_data[3].elv,
			   pack->sat_data[3].azimuth,
			   pack->sat_data[3].sig);
}

int
nmea_gen_GPRMC(char *buff, int buff_sz, nmeaGPRMC *pack)
{
	return nmea_printf(
	    buff,
	    buff_sz,
...
int
nmea_gen_GPVTG(char *buff, int buff_sz, nmeaGPVTG *pack)
{
	return nmea_printf(buff,
			   buff_sz,
...
void
nmea_info2GPGGA(const nmeaINFO *info, nmeaGPGGA *pack)
{
...
void
nmea_info2GPGSA(const nmeaINFO *info, nmeaGPGSA *pack)
{
	int it;
	...
	{
		pack->sat_prn[it] =
		    ((info->satinfo.sat[it].in_use) ? info->satinfo.sat[it].id
						    : 0);
	}
}

int
nmea_gsv_npack(int sat_count)
{
	int pack_count =
	    lrint(ceil(((double)sat_count) / (double)NMEA_SATINPACK));
...
void
nmea_info2GPGSV(const nmeaINFO *info, nmeaGPGSV *pack, int pack_idx)
{
	int sit, pit;
	...
	for (pit = 0, sit = pack->pack_index * NMEA_SATINPACK;
	     pit < NMEA_SATINPACK;
	     ++pit, ++sit)
...
void
nmea_info2GPRMC(const nmeaINFO *info, nmeaGPRMC *pack)
{
...
void
nmea_info2GPVTG(const nmeaINFO *info, nmeaGPVTG *pack)
{
...
int
nmea_generate(char *buff, int buff_sz, const nmeaINFO *info, int generate_mask)
{
	...
	while (pack_mask)
	{
		if (pack_mask & GPGGA)
		{
			...
			gen_count += nmea_gen_GPGGA(
			    buff + gen_count, buff_sz - gen_count, &gga);
			pack_mask &= ~GPGGA;
		}
		else if (pack_mask & GPGSA)
		{
			nmea_info2GPGSA(info, &gsa);
			gen_count += nmea_gen_GPGSA(
			    buff + gen_count, buff_sz - gen_count, &gsa);
			pack_mask &= ~GPGSA;
		}
		else if (pack_mask & GPGSV)
		{
			...
			for (gsv_it = 0;
			     gsv_it < gsv_count && buff_sz - gen_count > 0;
			     ++gsv_it)
			{
				nmea_info2GPGSV(info, &gsv, gsv_it);
				gen_count += nmea_gen_GPGSV(buff + gen_count,
							    buff_sz - gen_count,
							    &gsv);
			}
			pack_mask &= ~GPGSV;
		...
		else if (pack_mask & GPRMC)
		{
			...
			gen_count += nmea_gen_GPRMC(
			    buff + gen_count, buff_sz - gen_count, &rmc);
			pack_mask &= ~GPRMC;
		}
		else if (pack_mask & GPVTG)
		{
			nmea_info2GPVTG(info, &vtg);
			gen_count += nmea_gen_GPVTG(
			    buff + gen_count, buff_sz - gen_count, &vtg);
			pack_mask &= ~GPVTG;
		}
		else
			break;
