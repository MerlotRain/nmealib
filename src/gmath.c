 ...
 */
double
nmea_calc_pdop(double hdop, double vdop)
{
	return sqrt(pow(hdop, 2) + pow(vdop, 2));
}

double
nmea_dop2meters(double dop)
{
	return (dop * NMEA_DOP_FACTOR);
}

double
nmea_meters2dop(double meters)
{
 ...
 */
double
nmea_distance(const nmeaPOS *from_pos, /**< From position in radians */
...
)
{
	...
	A = 1 + sqr_u / 16384 *
		    (4096 + sqr_u * (-768 + sqr_u * (320 - 175 * sqr_u)));
 ...
 */
int
nmea_move_horz(const nmeaPOS *start_pos, /**< Start position in radians */
...
)
{
	...
	distance /= NMEA_EARTHRADIUS_KM; /* Angular distance covered on earth's
					    surface */
...
)
{
	...
	double sigma_initial, sigma, sigma_prev, sin_sigma, cos_sigma,
	    cos_2_sigmam, sqr_cos_2_sigmam, delta_sigma;
	int remaining_steps;
 ...
 */
void
nmea_info2pos(const nmeaINFO *info, nmeaPOS *pos)
{
 ...
 */
void
nmea_pos2info(const nmeaPOS *pos, nmeaINFO *info)
{
