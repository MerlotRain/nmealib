...
static double
nmea_random(double min, double max)
{
...
int
nmea_gen_init(nmeaGENERATOR *gen, nmeaINFO *info)
{
	int RetVal = 1;
...
int
nmea_gen_loop(nmeaGENERATOR *gen, nmeaINFO *info)
{
	int RetVal = 1;
...
int
nmea_gen_reset(nmeaGENERATOR *gen, nmeaINFO *info)
{
	int RetVal = 1;
...
void
nmea_gen_destroy(nmeaGENERATOR *gen)
{
	if (gen->next)
	{
...
void
nmea_gen_add(nmeaGENERATOR *to, nmeaGENERATOR *gen)
{
	if (to->next)
...
static int
nmea_igen_noise_init(nmeaGENERATOR *gen __attribute__((unused)),
...
static int
nmea_igen_noise_loop(nmeaGENERATOR *gen __attribute__((unused)), nmeaINFO *info)
{
	int it;
	int in_use;
	...
	for (it = 0; it < 12; ++it)
	{
		...
		info->satinfo.sat[it].in_use = in_use =
		    lrint(nmea_random(0, 3));
		info->satinfo.sat[it].elv = lrint(nmea_random(0, 90));
		info->satinfo.sat[it].azimuth = lrint(nmea_random(0, 359));
		info->satinfo.sat[it].sig =
		    (int)(in_use ? nmea_random(40, 99) : nmea_random(0, 40));
...
static int
nmea_igen_noise_reset(nmeaGENERATOR *gen __attribute__((unused)),
		      nmeaINFO *info __attribute__((unused)))
{
	return 1;
}
...
static int
nmea_igen_static_loop(nmeaGENERATOR *gen __attribute__((unused)),
		      nmeaINFO *info)
{
...
static int
nmea_igen_static_reset(nmeaGENERATOR *gen __attribute__((unused)),
		       nmeaINFO *info)
{
...
static int
nmea_igen_static_init(nmeaGENERATOR *gen, nmeaINFO *info)
{
	info->sig = 3;
	info->fix = 3;
...
static int
nmea_igen_rotate_loop(nmeaGENERATOR *gen __attribute__((unused)),
		      nmeaINFO *info)
{
	int it;
...
static int
nmea_igen_rotate_reset(nmeaGENERATOR *gen __attribute__((unused)),
		       nmeaINFO *info)
{
	int it;
	double deg = 360.0 / 8;
...
static int
nmea_igen_rotate_init(nmeaGENERATOR *gen, nmeaINFO *info)
{
	info->sig = 3;
	info->fix = 3;
...
static int
nmea_igen_pos_rmove_init(nmeaGENERATOR *gen __attribute__((unused)),
			 nmeaINFO *info)
{
	info->sig = 3;
	info->fix = 3;
...
static int
nmea_igen_pos_rmove_loop(nmeaGENERATOR *gen __attribute__((unused)),
			 nmeaINFO *info)
{
	nmeaPOS crd;
...
static int
nmea_igen_pos_rmove_destroy(nmeaGENERATOR *gen __attribute__((unused)))
{
	return 1;
};
...
static nmeaGENERATOR *
__nmea_create_generator(int type, nmeaINFO *info)
{
	nmeaGENERATOR *gen = 0;
	...
	default:
		...
		nmea_gen_add(
		    gen, __nmea_create_generator(NMEA_GEN_POS_RANDMOVE, info));
		break;
	};
...
nmeaGENERATOR *
nmea_create_generator(int type, nmeaINFO *info)
{
...
void
nmea_destroy_generator(nmeaGENERATOR *gen)
{
